...
Pure-function classifier for the terminal output of an autonomous run.
Lifecycle-state interpretation is supplied through the
:class:`LifecycleStateRegistry` interface so the same classifier handles
default generic vocabulary and any custom registry.
...
- :data:`STALL_WAITING_FOR_CONTINUE` — output is a prompt-style question
  asking for a human reply
...
"""The final output is a prompt-style question asking the operator to
type ``Continue`` (or ``yes``) before proceeding.
"""
...
def is_terminal_lifecycle_state(
    ...
    project-branded strings are rejected; callers must use the canonical
    identifier from their registry.
    """
...
def classify_terminal_message(
    ...
    vocabulary is project-specific. If ``registry`` is ``None``, the
    default generic registry is used.
    """
    ...
    checkpoint_path = _extract_value(text, "checkpoint") or _extract_value(text, "ckpt")
    ...
    # Terminal (or parked) state present.
