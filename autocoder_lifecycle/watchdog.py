...
Pure dataclass watchdog with injected timestamps and a generic pending-action
classifier. No clock reads, no GitHub calls, no filesystem I/O.
...
- :data:`STALL_RISK` — verdict returned when the watchdog detects a stall
  risk
- :data:`WATCHDOG_PROGRESS_REQUIRED` — verdict returned when the watchdog
  is idle beyond the budget and last_progress_at is not recent enough
"""
...
class WatchdogState:
    ...
    ``next_action`` and ``checkpoint_path`` are optional. ``terminal_state``
    is supplied as a string; the watchdog consults the supplied
...
def evaluate_watchdog(
    ...
    *,
    ...
    - ``STALL_RISK`` if the runner has both a ``next_action`` AND a
      ...
      action.
    - ``STALL_RISK`` if the runner has a recognized terminal/parked
      state but is still attempting to work; documented as a hard stop.
    - ``STALL_RISK`` if the phase timer exceeded ``max_phase_seconds``
      without terminal state.
    - ``STALL_RISK`` if ``caller_exhaustion_classifier`` returns a stall
      verdict.
    - ``WATCHDOG_PROGRESS_REQUIRED`` if idle > ``max_idle_seconds`` and
      no terminal state.
    ...
    # If runner has terminal/parked state, watchdog should not be evaluating.
    if state.terminal_state is not None and registry.is_terminal_or_parked(state.terminal_state):
        # Caller-supplied exhaustion classifier may still override.
    ...
    # Caller-supplied exhaustion classifier.
    verdict = (exhaustion_classifier or _default_exhaustion_classifier)(state, now)
    if verdict is not None:
        return verdict
